...
public class scheduleOfClassServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Transcript tst=service.getTranscript(ssn,scs.getSectionsOffered());// 找出该学期已选课程
		...
		for(int i=0;i<tst.getTranscriptEntries().size();i++){
			 jo=new JSONObject();
			 jo.put("tid",tst.getTranscriptEntries().get(i).getSection().getSectionNo() );
				jo.put("enrolled",tst.getTranscriptEntries().get(i).getSection().getFullSectionInfo());
			ja.put(jo);
		}
